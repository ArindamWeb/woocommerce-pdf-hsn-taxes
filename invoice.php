/*wp-content/plugins/woocommerce-pdf-invoices-packing-slips/templates/Simple/invoice.php*/
...
<tr>
    <?php
    foreach ( wpo_wcpdf_get_simple_template_default_table_headers( $this ) as $column_class => $column_title ) :

        // Print Product column
        if ( $column_class === 'product' ) :
            ?>
            <th class="product"><?php echo esc_html( $column_title ); ?></th>
            <th class="hsn">HSN</th>
            <?php
            continue;
        endif;
...
<td class="hsn">
    ...
    if ( ! empty( $item['product_id'] ) ) {
        ...
        if ( $product ) {
            $hsn = $product->get_attribute( 'hsn' ); // attribute slug
            echo $hsn ? esc_html( $hsn ) : '-';
        } else {
            echo '-';
        }
    } else {
